...
async def work(url_search, title, price, publication_auto, view_auto, modelURLs, api_token_bot, chat_id, cycle):
    try:
        data_list=get_urls(url_search)
        for elem in data_list:
            if not await tortoise_methods.get_all_from_DB(modelURLs, elem['href']):
                await tortoise_methods.add_to_DB(modelURLs, elem['href'])
                price_info=get_data_from_url(elem['href'])
                # print(elem)
                data_to_tg = elem['href'] + '\n' + elem['price'] + '\n' + price_info
                send_telegram(api_token_bot, chat_id, data_to_tg)
